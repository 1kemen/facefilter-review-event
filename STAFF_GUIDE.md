...
리뷰이벤트 상품은 **발급일로부터 30일** 동안 사용할 수 있습니다. 발급일은 고객이 뽑기를 확정한 날이고, 만료일 당일까지는 사용할 수 있습니다.

| 화면 | 어디에 보이는지 |
| --- | --- |
| 고객 결과 화면 | 상품 티켓 안에 `유효기간 YYYY.MM.DD까지`와 안내문. 캡처하면 날짜도 같이 남습니다. |
| 참여자 표 | 상품 칸. 만료됐거나 1주 안쪽으로 남은 건에만 배지가 붙습니다. |
| 참여자 상세 | `완료 상태`의 `상품 유효기간: YYYY.MM.DD까지 · D-n`. 차트 메모와 CSV에도 같은 값이 들어갑니다. |
| 지급/사용 현황 | 미사용 시술권 목록의 `유효기간` 열. 만료된 건은 붉게 표시되고 상단에 만료 건수가 나옵니다. |

유효기간이 지난 시술권을 들고 오신 경우 임의로 사용처리하지 말고 관리자에게 문의합니다.
...
```text
받으신 상품은 발급일로부터 30일 안에 사용하시면 됩니다.
결과 화면에 유효기간 날짜가 같이 적혀 있으니 화면 캡처해 두시면 편해요.
```
