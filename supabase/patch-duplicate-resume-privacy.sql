-- Prevent exposing an existing participant payload when only the review nickname matches.
-- Same-day duplicate resume returns the old payload only when name + phone also match.
...
begin
  ...
  if found then
    ...
    perform public.ff_write_audit(
      ...
      v_existing.id,
      p_session_id,
      case when v_resume_allowed then v_existing.id else null end,
      jsonb_build_object('visitDate', p_visit_date, 'resumeAllowed', v_resume_allowed)
    );
